...
def main():
    with st.sidebar:
        ...
        quantity = st.number_input('Enter Quantity', min_value=0, key='ym2')
        average_price = st.number_input('Enter Average Price ($)', min_value=100, key='ym3')

        # 수정/추가 버튼
        if st.button('Edit/Add Stock', key='ym4'):
            edit_or_add_stock(selected_user, selected_stock, quantity, average_price)
    with col1:
        st.title('YM')
...
def load_portfolio(FILE_NAME):
    ...
    try:
        ...
        portfolio = pd.DataFrame(columns=['Name', 'Quantity', 'Average Price'])
    return portfolio

def edit_or_add_stock(selected_user, selected_stock, quantity, average_price):
    """
    ...
    if selected_stock in portfolio['Name'].values:
        ...
        if quantity == 0:
            ...
            portfolio.loc[portfolio['Name'] == selected_stock, ['Quantity', 'Average Price']] = quantity, average_price
    else:
        # 선택한 종목이 포트폴리오에 없는 경우 추가
        new_stock = pd.DataFrame({'Name': [selected_stock], 'Quantity': [quantity], 'Average Price': [average_price]})
...
def draw_pie_chart(portfolio):
    ...
    if not portfolio.empty:
        ...
        amounts = portfolio['Amount'].tolist()
        ...
        # 전체 포트폴리오의 총 금액 계산
        portfolio['Total Amount'] = portfolio['Quantity'] * portfolio['Average Price']
        total_portfolio_amount = portfolio['Total Amount'].sum()
